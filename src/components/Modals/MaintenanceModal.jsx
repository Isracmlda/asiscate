...
export function MaintenanceModal({
  ...
  handleOpenStudentReadOnly,
  handleDeleteStudent,
  handleAddStudent,
  handleGenerateStudentQr,
  handleExportGroupRosterToPdf,
  handleExportGroupRosterToExcel,
  buildWhatsAppLink,
  cardBgClass,
  inputBgClass,
  themeMode = 'dark'
    ...
    <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 flex justify-center items-center p-4">
      <div className={`${cardBgClass} rounded-2xl max-w-2xl w-full p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto`}>
        <div className="flex justify-between items-start gap-3">
          <div className="flex flex-wrap items-start gap-3 sm:gap-5 min-w-0">
            <div>
        ...
        <div className="space-y-2 border-b border-slate-700 pb-2">
          <div className="flex flex-wrap justify-end gap-2">
            <button type="button" onClick={() => handleExportGroupRosterToPdf?.(maintenanceGroup.id)} className="px-3 py-1.5 rounded-lg text-xs font-bold bg-rose-700 hover:bg-rose-800 text-white">📄 Descargar PDF</button>
            <button type="button" onClick={() => handleExportGroupRosterToExcel?.(maintenanceGroup.id)} className="px-3 py-1.5 rounded-lg text-xs font-bold bg-emerald-700 hover:bg-emerald-800 text-white">📊 Descargar Excel</button>
          </div>
        </div>
        ...
        {maintenanceMode === 'view' ? (
          ...
          <div className="space-y-3">
            {visibleStudents.filter(student => student.groupId === maintenanceGroup.id).map(student => {
              const displayName = student.fullName || student.name || 'Sin nombre';
              return (
                <div key={student.id} className="flex items-center justify-between border-b border-slate-700 pb-2 gap-2">
                  <button type="button" onClick={() => handleOpenStudentReadOnly?.(student)} className="text-sm font-semibold text-left hover:text-sky-400 hover:underline truncate">{displayName}</button>
                  <div className="flex gap-2 shrink-0">
                    <button onClick={() => handleStartEditStudent(student)} className="px-3 py-1.5 text-xs bg-slate-700 text-white hover:bg-slate-600 rounded-lg font-bold">Editar</button>
                    <button onClick={() => handleDeleteStudent(student.id)} className="px-3 py-1.5 text-xs text-rose-700 dark:text-rose-300 bg-rose-100 dark:bg-rose-500/10 rounded-lg font-bold">Eliminar</button>
                  </div>
                </div>
              );
            })}
